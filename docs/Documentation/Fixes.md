...
`docs/lib/` contains two vendored third-party libraries that carry in-tree modifications. `paged.browser.js` is a patched copy of paged.js v0.4.3 (MIT); the thirteen `fast-*.mjs` files are side-effecting shims applied to pdf-lib's live exports before each PDF process phase. This section documents every change: what the upstream behaviour was, why it was unsuitable for the headless-Chromium pipeline, and what was changed.
