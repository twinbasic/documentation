...
- **Page-count drift.** `runBuild()` ends with `if (pages.length < 836) process.exitCode = 1`, printing `WARN: page count <n> below baseline 836`. It is a collapse alarm rather than a drift check: it compares against a fixed constant, not against the previous build, and `discover()` returns 908 pages today --- every `.md` and `.html` under `docs/` with a parseable frontmatter block, after `_config.yml`'s `exclude:`. So 72 pages can vanish in one build without crossing the floor, and the margin grows with every page added, because the constant does not move with it. One dropped page never trips the guard, and neither would a repeat of the 37-page `AppGlobalClassObject/_App/` loss that [Authoring](Authoring#what-may-live-in-docs) describes. What reports a single page depends on how it went missing: a page that loses its frontmatter --- a UTF-8 BOM in front of the `---`, or any line before it --- is reclassified as a static file, and the publish-policy sweep below then aborts the build naming the path, because `.md` is in neither extension set; a page dropped by an `exclude:` pattern is never seen at all, and the only report is the link check flagging broken links into it, so a page nothing links to disappears without a message. Nav integrity covers the remaining case only indirectly --- it fires when a *parent* disappears and strands its children, not when a leaf does.
