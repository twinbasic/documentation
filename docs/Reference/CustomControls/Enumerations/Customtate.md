...
> [!NOTE]
> The name **Customtate** appears to be a typo for "CustomState" preserved from an early draft of the package. The enum is not referenced by any of the eight concrete `Waynes…` controls; the actual minimized / normal / maximized state of a [**WaynesForm**](../WaynesForm/) is controlled by the parallel [**WindowState**](WindowState) enum, which has identical members. Treat **Customtate** as reserved.
