...
| Class | Role |
|-------|------|
| [Anchors](Anchors) | which sides of a control are attached to the container when it resizes |
| [Borders](Borders) | an array of border strokes drawn around a region; contains the per-stroke `Border` sub-object |
| [Corners](Corners) | the four corner shapes / radii of a region; contains the per-corner `Corner` sub-object |
| [Fill](Fill) | the colour or gradient that paints a region; contains the `FillColorPoint` and `FillColorPoints` sub-objects that hold the gradient stops |
