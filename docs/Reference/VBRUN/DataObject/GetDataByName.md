...
> [!NOTE]
> **GetDataByName** is a twinBASIC addition; it has no equivalent in VB6. Use it when the consumer side knows the format only by its registered name and does not have the corresponding numeric identifier handy. For the standard built-in formats, [**GetData**](GetData) with a **ClipboardConstants** value is more direct.
