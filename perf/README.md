...
paged.js + headless Chromium + pdf-lib (see `book.bat`, which
invokes `book/render-book.mjs`). The pipeline was historically driven
...
- `--parallel-deflate` -- swap `pdfDoc.save()` for `parallelSave`
  from [book/lib/parallel-deflate.mjs](../book/lib/parallel-deflate.mjs),
...
- `--fast-decode-name` -- inject
  [book/lib/fast-decode-name.mjs](../book/lib/fast-decode-name.mjs), a
...
- `--fast-number-to-string` -- inject
  [book/lib/fast-number-to-string.mjs](../book/lib/fast-number-to-string.mjs),
...
- `--fast-size-in-bytes` -- inject
  [book/lib/fast-size-in-bytes.mjs](../book/lib/fast-size-in-bytes.mjs),
...
- `--fast-array-onebuf` -- inject
  [book/lib/fast-array-onebuf.mjs](../book/lib/fast-array-onebuf.mjs).
...
- `--measure-pass` -- inject
  [book/lib/measure-pass.mjs](../book/lib/measure-pass.mjs), the
...
- `--fast-parse-object` -- inject
  [book/lib/fast-parse-object.mjs](../book/lib/fast-parse-object.mjs),
...
- `--fast-parse-name` -- inject
  [book/lib/fast-parse-name.mjs](../book/lib/fast-parse-name.mjs),
...
- `--fast-sync-load` -- inject
  [book/lib/fast-sync-load.mjs](../book/lib/fast-sync-load.mjs),
...
- `--fast-pdfnumber-pool` -- inject
  [book/lib/fast-pdfnumber-pool.mjs](../book/lib/fast-pdfnumber-pool.mjs),
...
| `detach-pages.js` | `Paged.Handler` that hides each completed page from the layout tree (registered against `finalizePage`). The shipping fix. Injected by default (both by `measure.mjs` and by `book.bat`); pass `--no-detach-pages` to measure the pre-fix baseline. |
...
| `ab-css.mjs` | CSS cost attribution for `docs/_site-pdf/assets/css/print.css` + `tb-highlight.css`. Renders the book per variant (full / drop-highlight / drop-print-extras / baseline-minimal) and reports **paired-difference** CPU sample-time across N pairs (default 3), with the baseline re-measured immediately before each variant pair to cancel machine-state drift. Pulls per-`Document::recalcStyle` / `LocalFrameView::performLayout` / `rebuildLayoutTree` / `ShapeText` total time from the embedded V8 cpu profile in the hybrid trace; prints mean ± SD per variant so noise-floor rows are visible. Auto-pins on Windows via `pin-cpu.mjs`. Optional `--per-print-section` adds one drop-print-`<section>` variant per `/* ---- ---- */` divider in print.css; individual sections of print.css turned out to be below the noise floor on this book, so off by default. |
...
| `probe-parallel.mjs` | N-shard `Promise.all` `page.pdf()` probe -- the cost-of-`pageRanges`-sharding measurement (see *`pageRanges` sharding: off the table for now* in [notes/06-microtasks-pageranges-css.md](notes/06-microtasks-pageranges-css.md)). One browser per shard, deliberately; see `probe-tabs-vs-procs.mjs`. Run with `node perf/probe-parallel.mjs [book.html] [--shards N]`. |
| `probe-tabs-vs-procs.mjs` | Browser-topology A/B for parallel `page.pdf()`: N tabs in one browser vs. N separate browsers, same book + paged.js + `pageRanges` split. Samples the chrome.exe tree during generate to count PrintCompositor utility processes. Establishes why `probe-parallel.mjs` launches a full browser per shard -- one browser pools the compositor below the shard count, costing ~33 % of generate wall clock (see *Tabs vs. processes* in [notes/06-microtasks-pageranges-css.md](notes/06-microtasks-pageranges-css.md)). Run with `node perf/probe-tabs-vs-procs.mjs tabs\|procs [--shards N]`; `PROD_ARGS=1` adds production's `--disable-gpu` pair. |
...
- same paged.js bundle: we vendor `book/lib/paged.browser.js`, taken
...
- same outline + metadata helpers: `book/lib/outline.mjs` and
  `book/lib/postprocesser.mjs` are MIT-licensed copies of
...
`book/render-book.mjs` and `perf/measure.mjs` share the same helpers
...
| [06-microtasks-pageranges-css.md](notes/06-microtasks-pageranges-css.md) | Following `RunMicrotasks` down to zero (chunker fully sync); why `pageRanges` sharding is off the table (and why, if it is ever revisited, each shard needs its own browser process rather than a tab -- one browser pools the PrintCompositor below the shard count, so the surplus shard queues and generate wall clock goes up ~33 %, while tabs reclaim no memory because each gets its own renderer anyway); CSS cost attribution showing print.css's individual sections are all below the noise floor. |
