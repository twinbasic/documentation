...
- `build-offline.bat` — produce **only** the offline copy, writing directly to `_site-offline/` (no `_site/` is generated). Layers `_config_offline.yml` over `_config.yml` to set `offline_build: true` (activates the plugin's standalone in-place mode) and override `also_build_offline: false`. Faster than the combined build when you don't need the online copy — Jekyll renders with `search_enabled: false` (the search markup never reaches the HTML at all, so no CSS hide hack is needed), `gh_edit_link: false`, `last_edit_timestamp: false`. Useful for shipping just the offline copy as a downloadable bundle.
